...
`);u=g.pop();for(const k of g)if(k.startsWith("data: ")){const h=k.substring(6);try{const E=JSON.parse(h);if(E.type==="metadata")r.manga.title=E.title,r.manga.alias=E.title,Ie();else if(E.type==="image"){const _=`/api/scrapers/proxy-cover?url=${encodeURIComponent(E.url)}`;r.images.push(_),ue()}else if(E.type==="error")d("Stream error: "+E.message,"error");else if(E.type==="done")break}catch(E){console.error("Parse error for SSE data:",E)}}}}catch(a){a.name!=="AbortError"&&(console.error("Preview stream error:",a),d("Stream failed: "+a.message,"error"))}finally{r._streamAbortController&&r._streamAbortController.signal===t&&(r._streamAbortController=null)}}async function _a(s=[]){console.log("[Reader] mount called with params:",s);let[e,t]=s,a=null;if(t&&t.includes("?")){const[i,o]=t.split("?");t=i,a=new URLSearchParams(o).get("version")}if(console.log("[Reader] mangaId:",e,"chapterNum:",t,"urlVersion:",a),!e||!t){T.go("/");return}const n=document.getElementById("app");if(r.loading=!0,console.log("[Reader] loading set to true, calling loadData..."),r.images=[],r.singlePageMode=!1,r._resumeScrollToPage=null,r.nextChapterImage=null,r.nextChapterNum=null,n.innerHTML=ye(),a)await ge(e,t,decodeURIComponent(a));else try{const i=await y.getBookmark(e),o=i.downloadedVersions||{},c=new Set(i.deletedChapterUrls||[]),l=o[parseFloat(t)];let u=[];if(Array.isArray(l)&&(u=l.filter(p=>!c.has(p))),u.length>1){const p=await Ia(u,t);if(p===null){T.go(`/manga/${e}`);return}await ge(e,t,p)}else u.length===1?await ge(e,t,u[0]):await ge(e,t)}catch(i){console.log("[Reader] Error in version check, falling back:",i),await ge(e,t)}if(n.innerHTML=ye(),console.log("[Reader] render called, loading:",r.loading,"manga:",!!r.manga,"images:",r.images.length),He(),r.mode==="webtoon"&&r._resumeScrollToPage!=null){const i=r._resumeScrollToPage;r._resumeScrollToPage=null,setTimeout(()=>{const o=document.getElementById("reader-content");if(o){const c=o.querySelectorAll("img");c[i]&&c[i].scrollIntoView({behavior:"auto",block:"start"})}},300)}}async function Aa(){console.log("[Reader] unmount called"),r._streamAbortController&&(r._streamAbortController.abort(),r._streamAbortController=null),r.isStreamingMode||(await Ae(),await fe()),document.body.classList.remove("reader-active"),document.removeEventListener("keydown",is),r.manga=null,r.chapter=null,r.images=[],r.loading=!0,r.singlePageMode=!1,r.isStreamingMode=!1,r._resumeScrollToPage=null,r._preloadCache=null}async function fe(){if(!(!r.manga||!r.chapter||r.manga.id==="gallery"||r.isStreamingMode))try{await y.updateChapterSettings(r.manga.id,r.chapter.number,{mode:r.mode,direction:r.direction,firstPageSingle:r.firstPageSingle,lastPageSingle:r.lastPageSingle})}catch(s){console.error("Failed to save settings:",s)}}async function os(s){try{const e=await y.getBookmark(s),t=e.downloadedChapters||[],a=new Set(e.readChapters||[]),n=e.readingProgress||{},i=e.downloadedVersions||{},o=[...t].sort((l,u)=>l-u);let c=null;for(const l of o){const u=n[l];if(u&&u.page<u.totalPages&&!a.has(l)){c=l;break}}if(c===null){for(const l of o)if(!a.has(l)){c=l;break}}if(c===null&&o.length>0&&(c=o[0]),c!==null){const l=i[c]||[],u=Array.isArray(l)?l[0]:l,p=u?`?version=${encodeURIComponent(u)}`:"";T.go(`/read/${s}/${c}${p}`)}else d("No downloaded chapters to read","info")}catch(e){d("Failed to continue reading: "+e.message,"error")}}const Pa={mount:_a,unmount:Aa,render:ye,continueReading:os},Ta="manga-offline",Ma=1,ht="images",je="chapters";let Me=null;function ls(){return new Promise((s,e)=>{if(Me)return s(Me);const t=indexedDB.open(Ta,Ma);t.onupgradeneeded=a=>{const n=a.target.result;n.objectStoreNames.contains(ht)||n.createObjectStore(ht),n.objectStoreNames.contains(je)||n.createObjectStore(je)},t.onsuccess=()=>{Me=t.result,s(Me)},t.onerror=()=>e(t.error)})}function Pt(s,e,t){return ls().then(a=>new Promise((n,i)=>{const l=a.transaction(s,"readwrite").objectStore(s).put(t,e);l.onsuccess=()=>n(),l.onerror=()=>i(l.error)}))}function Ra(s){return ls().then(e=>new Promise((t,a)=>{const o=e.transaction(s,"readonly").objectStore(s).getAllKeys();o.onsuccess=()=>t(o.result),o.onerror=()=>a(o.error)}))}function Na(s,e){return`${s}:${e}`}function Da(s,e,t){return`${s}:${e}:${t}`}function qa(s){const e=s.split(":");return{mangaId:e[0],chapterNum:parseFloat(e[1])}}async function Fa(s,e,t=null){const a=await y.get(`/bookmarks/${s}/chapters/${e}/reader-images`);if(!a||!a.images||a.images.length===0)throw new Error("No images found for this chapter");const n=a.images,i=n.length;let o=0;const c=y.getToken();for(let u=0;u<n.length;u++){const p=typeof n[u]=="string"?n[u]:n[u].url,v=p.startsWith("http")?p:`${window.location.origin}${p}`;try{const g=await fetch(v,{headers:c?{Authorization:`Bearer ${c}`}:{}});if(!g.ok)throw new Error(`HTTP ${g.status}`);const k=await g.blob();await Pt(ht,Da(s,e,p),k),o++,t&&t(o,i)}catch(g){console.error(`[Offline] Failed to cache image ${u+1}/${i}:`,g)}}const l={mangaId:s,chapterNum:e,imageUrls:n.map(u=>typeof u=="string"?u:u.url),savedAt:Date.now(),imageCount:o};return await Pt(je,Na(s,e),l),{success:!0,imageCount:o}}async function Oa(s){const e=await Ra(je),t=[];for(const a of e)if(!a.startsWith("auto-offline-")&&a.startsWith(`${s}:`)){const{chapterNum:n}=qa(a);t.push(n)}return t}"serviceWorker"in navigator&&navigator.serviceWorker.addEventListener("message",async s=>{var e;if(((e=s.data)==null?void 0:e.type)==="sync-offline"){const t=s.data.mangaId;console.log(`[Offline] Auto-sync triggered for manga ${t}`);try{await Ua(t)}catch(a){console.error("[Offline] Auto-sync failed:",a)}}});async function Ua(s){try{const e=await y.getBookmark(s);if(!e)return;const t=e.downloadedChapters||[],a=await Oa(s),n=t.filter(i=>!a.includes(i));console.log(`[Offline] ${n.length} new chapters to sync for ${e.alias||e.title}`);for(const i of n)await Fa(s,i),console.log(`[Offline] Auto-synced chapter ${i}`)}catch(e){console.error("[Offline] Sync error:",e)}}const Le=50;let b={manga:null,categories:[],currentPage:0,filter:"all",loading:!0,selectionMode:!1,selected:new Set,activeVolume:null,activeVolumeId:null,cbzFiles:[],manageChapters:!1,offlineChapters:new Set,isAutoOffline:!1};function Va(s){return s.autoCheck===!0?`<button class="btn btn-primary" id="schedule-btn">⏰ ${s.checkSchedule==="weekly"?`${(s.checkDay||"monday").charAt(0).toUpperCase()+(s.checkDay||"monday").slice(1)} ${s.checkTime||"06:00"}`:s.checkSchedule==="daily"?`Daily ${s.checkTime||"06:00"}`:"Every 6h"}</button>`:'<button class="btn btn-secondary" id="schedule-btn">⏰ Schedule</button>'}function Ha(s){const e=s.autoCheck===!0,t=s.checkSchedule||"daily",a=s.checkDay||"monday",n=s.checkTime||"06:00",i=s.autoDownload||!1;return`
  ...
  `}function mt(){var C;if(b.loading)return`
    ...
    `;const e=s.alias||s.title,t=s.chapters||[],a=new Set(s.downloadedChapters||[]),n=new Set(s.readChapters||[]),i=new Set(s.excludedChapters||[]),o=new Set(s.deletedChapterUrls||[]),c=s.volumes||[],l=new Set;c.forEach(f=>{(f.chapters||[]).forEach($=>l.add($))});let u;b.filter==="hidden"?u=t.filter(f=>i.has(f.number)||o.has(f.url)):u=t.filter(f=>!i.has(f.number)&&!o.has(f.url));const p=u.filter(f=>!l.has(f.number));let v=[];if(b.activeVolume){const f=new Set(b.activeVolume.chapters||[]);v=u.filter($=>f.has($.number))}else v=p;const g=new Map;v.forEach(f=>{g.has(f.number)||g.set(f.number,[]),g.get(f.number).push(f)});let k=Array.from(g.entries()).sort((f,$)=>f[0]-$[0]);b.filter==="downloaded"?k=k.filter(([f])=>a.has(f)):b.filter==="not-downloaded"?k=k.filter(([f])=>!a.has(f)):b.filter==="main"?k=k.filter(([f])=>Number.isInteger(f)):b.filter==="extra"&&(k=k.filter(([f])=>!Number.isInteger(f)));const h=Math.max(1,Math.ceil(k.length/Le));b.currentPage>=h&&(b.currentPage=Math.max(0,h-1));const E=b.currentPage*Le,I=[...k.slice(E,E+Le)].reverse(),P=g.size,M=[...g.keys()].filter(f=>a.has(f)).length;n.size;let N="";if(b.activeVolume){const f=b.activeVolume;let $=null;f.local_cover?$=`/api/public/covers/${s.id}/${encodeURIComponent(f.local_cover.split(/[/\\]/).pop())}`:f.cover&&($=f.cover),N=`
      ${se()}
      <div class="container">
        <div class="manga-detail">
          <div class="manga-detail-header">
            <div class="manga-detail-cover">
              ${$?`<img src="${$}" alt="${f.name}">`:'<div class="placeholder">📚</div>'}
            </div>
    ...
    <div class="chapter-group" data-chapter="${s}">
      <div class="${I}" data-num="${s}" style="${M?"opacity: 0.7":""}">
        ...
        <div class="chapter-actions">
          ...
          ${!M&&h?u.has(($=g[0])==null?void 0:$.url)?`<button class="btn-icon small warning" data-action="unhide-chapter" data-num="${s}" data-url="${h}" title="Unhide Chapter">↩️</button>`:`<button class="btn-icon small" data-action="hide-chapter" data-num="${s}" data-url="${h}" title="Hide Chapter">👁️‍🗨️</button>`:""}
  ...
  `}function Ya(){var a,n,i,o,c,l,u,p,v,g,k,h,E,_,I,P,M,N,C,f,$,R;const s=document.getElementById("app"),e=b.manga;if(!e)return;(a=document.getElementById("back-btn"))==null||a.addEventListener("click",()=>T.go("/")),(n=document.getElementById("back-library-btn"))==null||n.addEventListener("click",()=>T.go("/")),s.querySelectorAll(".artist-link").forEach(m=>{m.addEventListener("click",w=>{w.preventDefault();const x=m.dataset.artist;x&&(localStorage.setItem("library_search",x),localStorage.removeItem("library_artist_filter"),T.go("/"))})}),(i=document.getElementById("continue-btn"))==null||i.addEventListener("click",()=>{os(e.id)}),(o=document.getElementById("download-all-btn"))==null||o.addEventListener("click",()=>{const m=document.getElementById("download-all-modal");m&&m.classList.add("open")}),(c=document.getElementById("confirm-download-all-btn"))==null||c.addEventListener("click",async()=>{var m;try{d("Queueing downloads...","info");const w=document.getElementsByName("download-version-mode");let x="single";for(const A of w)A.checked&&(x=A.value);(m=document.getElementById("download-all-modal"))==null||m.classList.remove("open");const L=await y.post(`/bookmarks/${e.id}/download`,{all:!0,versionMode:x});L.chaptersCount>0?d(`Download queued: ${L.chaptersCount} versions`,"success"):d("Already have these chapters downloaded","info")}catch(w){d("Failed to download: "+w.message,"error")}}),(l=document.getElementById("check-updates-btn"))==null||l.addEventListener("click",async()=>{try{d("Checking for updates...","info"),await y.post(`/bookmarks/${e.id}/quick-check`),d("Check complete!","success")}catch(m){d("Check failed: "+m.message,"error")}}),(u=document.getElementById("schedule-btn"))==null||u.addEventListener("click",()=>{const m=document.getElementById("schedule-modal");m&&m.classList.add("open")}),(p=document.getElementById("schedule-type"))==null||p.addEventListener("change",m=>{const w=document.getElementById("schedule-day-group");w&&(w.style.display=m.target.value==="weekly"?"":"none")}),(v=document.getElementById("save-schedule-btn"))==null||v.addEventListener("click",async()=>{var m;try{const w=document.getElementById("schedule-type").value,x=document.getElementById("schedule-day").value,L=document.getElementById("schedule-time").value,A=document.getElementById("auto-download-toggle").checked;await y.updateAutoCheckSchedule(e.id,{enabled:!0,schedule:w,day:x,time:L,autoDownload:A}),b.manga.checkSchedule=w,b.manga.checkDay=x,b.manga.checkTime=L,b.manga.autoDownload=A,(m=document.getElementById("schedule-modal"))==null||m.classList.remove("open"),U([e.id]),d("Schedule updated","success")}catch(w){d("Failed to save schedule: "+w.message,"error")}}),(g=document.getElementById("disable-schedule-btn"))==null||g.addEventListener("click",async()=>{var m;try{await y.toggleAutoCheck(e.id,!1),b.manga.autoCheck=!1,b.manga.checkSchedule=null,b.manga.checkDay=null,b.manga.checkTime=null,b.manga.nextCheck=null,(m=document.getElementById("schedule-modal"))==null||m.classList.remove("open"),U([e.id]),d("Auto-check disabled","success")}catch(w){d("Failed to disable: "+w.message,"error")}}),(k=document.getElementById("refresh-btn"))==null||k.addEventListener("click",async()=>{const m=document.getElementById("refresh-btn");try{m.disabled=!0,m.textContent="⏳ Checking...",d("Checking for updates...","info"),await y.post(`/bookmarks/${e.id}/check`),await W(e.id),U([e.id]),d("Check complete!","success")}catch(w){d("Check failed: "+w.message,"error"),m&&(m.disabled=!1,m.textContent="🔄 Refresh")}}),(h=document.getElementById("scan-folder-btn"))==null||h.addEventListener("click",async()=>{var w,x;const m=document.getElementById("scan-folder-btn");try{m.disabled=!0,m.textContent="⏳ Scanning...",d("Scanning folder...","info");const L=await y.scanBookmark(e.id);await W(e.id),U([e.id]);const A=((w=L.addedChapters)==null?void 0:w.length)||0,O=((x=L.removedChapters)==null?void 0:x.length)||0;A>0||O>0?d(`Scan complete: ${A} added, ${O} removed`,"success"):d("Scan complete: No changes","info")}catch(L){d("Scan failed: "+L.message,"error")}finally{m&&(m.disabled=!1,m.textContent="📁 Scan Folder")}}),document.querySelectorAll("[data-cbz-path]").forEach(m=>{m.addEventListener("click",async()=>{const w=decodeURIComponent(m.dataset.cbzPath),x=parseInt(m.dataset.cbzChapter)||1,L=m.dataset.cbzExtracted==="true",A=prompt("Enter chapter number for extraction:",String(x));if(!A)return;const O=parseFloat(A);if(isNaN(O)){d("Invalid chapter number","error");return}try{m.disabled=!0,m.textContent="Extracting...",d("Extracting CBZ...","info"),await y.extractCbz(e.id,w,O,{forceReExtract:L}),d("CBZ extracted successfully!","success"),await W(e.id),U([e.id])}catch(B){d("Extract failed: "+B.message,"error")}finally{m.disabled=!1,m.textContent=L?"Re-Extract":"Extract"}})}),(E=document.getElementById("edit-btn"))==null||E.addEventListener("click",async()=>{const m=document.getElementById("edit-manga-modal");if(m){document.getElementById("edit-alias-input").value=e.alias||"",window._selectedCoverPath=null;try{const[w,x]=await Promise.all([y.getAllArtists(),y.getAllCategories()]),L=document.getElementById("artist-list"),A=document.getElementById("category-list");window._allArtists=w,window._allCategories=x,L&&(L.innerHTML=w.map(D=>`<option value="${D}">`).join("")),A&&(A.innerHTML=x.map(D=>`<option value="${D}">`).join(""));const O=document.getElementById("edit-artist-input"),B=document.getElementById("edit-categories-input");O==null||O.addEventListener("input",()=>{const D=O.value.toLowerCase(),V=O.value.lastIndexOf(","),J=O.value.substring(V+1).trim().toLowerCase();if(J.length>0&&window._allArtists){const H=window._allArtists.filter(ie=>ie.toLowerCase().includes(J));if(L&&H.length>0){const ie=V>=0?O.value.substring(0,V+1)+" ":"";L.innerHTML=H.map(me=>`<option value="${ie}${me}">`).join("")}}}),B==null||B.addEventListener("input",()=>{const D=B.value.lastIndexOf(","),V=B.value.substring(D+1).trim().toLowerCase();if(V.length>0&&window._allCategories){const J=window._allCategories.filter(H=>H.toLowerCase().includes(V));if(A&&J.length>0){const H=D>=0?B.value.substring(0,D+1)+" ":"";A.innerHTML=J.map(ie=>`<option value="${H}${ie}">`).join("")}}})}catch(w){console.error("Failed to load artists/categories:",w)}m.classList.add("open")}}),(_=document.getElementById("save-manga-btn"))==null||_.addEventListener("click",async()=>{var m;try{const w=document.getElementById("edit-alias-input").value.trim(),x=document.getElementById("edit-artist-input").value.trim(),L=document.getElementById("edit-categories-input").value.trim(),A=x?x.split(",").map(B=>B.trim()).filter(B=>B):[],O=L?L.split(",").map(B=>B.trim()).filter(B=>B):[];await y.updateBookmark(e.id,{alias:w||null}),await y.setBookmarkArtists(e.id,A),await y.setBookmarkCategories(e.id,O),window._selectedCoverPath&&await y.setBookmarkCoverFromImage(e.id,window._selectedCoverPath),b.manga.alias=w||null,b.manga.artists=A,b.manga.categories=O,(m=document.getElementById("edit-manga-modal"))==null||m.classList.remove("open"),U([e.id]),d("Manga updated","success")}catch(w){d("Failed to update: "+w.message,"error")}}),(I=document.getElementById("change-cover-btn"))==null||I.addEventListener("click",async()=>{try{d("Loading images...","info");const m=await y.getFolderImages(e.id);if(m.length===0){d("No images found in manga folder","warning");return}const w=document.createElement("div");w.id="cover-select-modal",w.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.8);z-index:10000;display:flex;align-items:center;justify-content:center;",w.innerHTML=`
      ...
      `,document.body.appendChild(w),document.getElementById("close-cover-modal").addEventListener("click",()=>w.remove()),w.addEventListener("click",x=>{x.target===w&&w.remove()}),w.querySelectorAll(".cover-option").forEach(x=>{x.addEventListener("click",()=>{window._selectedCoverPath=x.dataset.path;const L=document.getElementById("cover-preview");L&&(L.innerHTML=`<img src="/api/proxy-image?path=${encodeURIComponent(window._selectedCoverPath)}" style="width:100%;height:100%;object-fit:cover;">`),w.remove(),d("Cover selected","success")})})}catch(m){d("Failed to load images: "+m.message,"error")}}),(P=document.getElementById("delete-manga-btn"))==null||P.addEventListener("click",()=>{const m=document.getElementById("delete-manga-modal");m&&m.classList.add("open")}),(M=document.getElementById("confirm-delete-manga-btn"))==null||M.addEventListener("click",async()=>{var w,x;const m=((w=document.getElementById("delete-files-toggle"))==null?void 0:w.checked)||!1;try{await y.deleteBookmark(e.id,m),(x=document.getElementById("delete-manga-modal"))==null||x.classList.remove("open"),d("Manga deleted","success"),T.go("/")}catch(L){d("Failed to delete: "+L.message,"error")}}),(N=document.getElementById("quick-check-btn"))==null||N.addEventListener("click",async()=>{const m=document.getElementById("quick-check-btn");try{m.disabled=!0,m.textContent="⏳ Checking...",d("Quick checking for updates...","info");const w=await y.post(`/bookmarks/${e.id}/quick-check`);await W(e.id),U([e.id]),w.newChaptersCount>0?d(`Found ${w.newChaptersCount} new chapter(s)!`,"success"):d("No new chapters found","info")}catch(w){d("Quick check failed: "+w.message,"error")}finally{m&&(m.disabled=!1,m.textContent="⚡ Quick Check")}}),(C=document.getElementById("source-label"))==null||C.addEventListener("click",async()=>{const m=document.getElementById("migrate-source-modal");if(m){m.classList.add("open");const w=document.getElementById("migrate-search-scraper");if(w&&w.options.length<=1)try{const x=await y.get("/scrapers/list");if(x.success){const L=x.scrapers.filter(A=>A.supportsSearch);w.innerHTML='<option value="all">All Sources</option>'+L.map(A=>`<option value="${A.name}">${A.name}</option>`).join(""),w.value="all"}}catch(x){console.warn("Failed to load scrapers:",x)}}});const t=async()=>{var O,B,D;const m=(B=(O=document.getElementById("migrate-search-input"))==null?void 0:O.value)==null?void 0:B.trim(),w=(D=document.getElementById("migrate-search-scraper"))==null?void 0:D.value;if(!m)return;const x=document.getElementById("migrate-search-loading"),L=document.getElementById("migrate-search-results"),A=document.getElementById("migrate-results-grid");x.style.display="block",L.style.display="none";try{const J=(await y.get(`/scrapers/search?q=${encodeURIComponent(m)}&scraper=${encodeURIComponent(w)}`)).results||[];J.length===0?A.innerHTML='<p class="text-muted" style="text-align: center; padding: 20px;">No results found</p>':(A.innerHTML=J.map(H=>{var me;const ie=(me=H.cover)!=null&&me.startsWith("/covers/")?H.cover:H.cover?`/api/scrapers/proxy-cover?url=${encodeURIComponent(H.cover)}`:"";return`
  ...
  `,he(),await ve(),oe(),Oe=setInterval(async()=>{await ve(),oe()},5e3),Z.downloadProgress=e=>{e.taskId&&F.downloads[e.taskId]&&(Object.assign(F.downloads[e.taskId],e),oe())},Z.downloadCompleted=e=>{ve().then(oe)},Z.queueUpdated=e=>{ve().then(oe)},z.on(K.DOWNLOAD_PROGRESS,Z.downloadProgress),z.on(K.DOWNLOAD_COMPLETED,Z.downloadCompleted),z.on(K.QUEUE_UPDATED,Z.queueUpdated)}function Un(){Oe&&(clearInterval(Oe),Oe=null),Z.downloadProgress&&z.off(K.DOWNLOAD_PROGRESS,Z.downloadProgress),Z.downloadCompleted&&z.off(K.DOWNLOAD_COMPLETED,Z.downloadCompleted),Z.queueUpdated&&z.off(K.QUEUE_UPDATED,Z.queueUpdated),Z={}}const Vn={mount:On,unmount:Un};class Hn{constructor(){this.container=null,this.scrapers=[],this.currentQuery="",this.currentTarget="all",this.isSearching=!1,this.results=[],this.viewMode="main",this.browseScraper=null,this.browseQuery="english",this.browseSort="popular-today",this.browsePage=1,this.browseTotalPages=1,this.isBrowsing=!1,this.browseResults=[],this.previewInfo=null,this.previewImages=[],this.previewIndex=0,this.infoAbortController=null}async mount(e){this.container=document.getElementById("app"),document.body.className="scrapers-mode",this.updateView(),await this.loadScrapers(),this.viewMode==="browse"&&this.browseScraper&&this.performBrowse();const t=window.location.hash.match(/\?q=([^&]*)/);t&&t[1]&&(this.currentQuery=decodeURIComponent(t[1]),this.updateView(),this.performSearch())}unmount(){this.infoAbortController&&(this.infoAbortController.abort(),this.infoAbortController=null),this.container.innerHTML="",document.body.className=""}async loadScrapers(){try{const e=await y.get("/scrapers/list");e.success&&(this.scrapers=e.scrapers,this.updateView())}catch(e){console.error("Failed to load scrapers",e)}}updateView(){this.render(),this.renderScraperList(),(this.results.length>0||this.isSearching)&&this.renderResults(),this.bindEvents()}render(){this.container.innerHTML=`
    ...
    `,i.disabled=!(e.url||e.galleryId);try{const o=await y.get(`/scrapers/info?url=${encodeURIComponent(e.url)}`,{signal:t});if(o.success&&o.info){this.previewInfo={...this.previewInfo,...o.info};let c="";o.info.tags&&o.info.tags.length>0&&(c=`
