...
class Downloader {
  ...
  // Get page counts for all downloaded chapters
  async getDownloadedChapterPageCounts(mangaTitle, alias = null) {
    const mangaDir = this.getMangaDir(mangaTitle, alias);
    const pageCounts = {};
      ...
      if (entry.isDirectory() && entry.name.startsWith('Chapter')) {
        ...
        if (match) {
          ...
          try {
            ...
            if (imageCount > 0) {
              if (!pageCounts[chapterNumber] || imageCount > pageCounts[chapterNumber]) {
                pageCounts[chapterNumber] = imageCount;
              }
            }
          } catch (e) {
  ...
  async findCbzFiles(mangaTitle, alias = null) {
    ...
    for (const entry of entries) {
      if (entry.isFile() && /\.(cbz|zip)$/i.test(entry.name)) {
  ...
  async scanAllMangaFolders() {
    ...
    for (const entry of entries) {
      ...
      const cbzFiles = folderContents.filter(e =>
        e.isFile() && /\.(cbz|zip)$/i.test(e.name)
      );
