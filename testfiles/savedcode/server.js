...
app.post('/api/bookmarks/:id/download', async (req, res) => {
  try {
    ...
    if (all) {
      // Download all chapters NOT already downloaded and NOT deleted
      ...
      // Download specific chapters (user explicitly chose, ignore deleted status)
      chaptersToDownload = chapters.filter(n => !bookmark.downloadedChapters.includes(n));
    }
  ...
  try {
    ...
    const localOnly = mangaFolders.filter(folder => {
      return !bookmarks.some(b =>
        downloader.sanitizeFileName(b.title) === folder.folderName ||
        (b.alias && downloader.sanitizeFileName(b.alias) === folder.folderName)
      );
    });
...
app.post('/api/bookmarks/:id/scan-local', async (req, res) => {
  try {
    ...
    // Update downloadedChapters to match reality
    await bookmarkDb.update(bookmark.id, {
    ...
    res.json({
      ...
      removedChapters,
      addedChapters,
      changed: removedChapters.length > 0 || addedChapters.length > 0
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
