...
function renderMangaDetail(manga, allCategories = [], currentPage = 0) {
  ...
  const wholeChapters = chapters.filter(c => Number.isInteger(c.number));
  const extraChapters = chapters.filter(c => !Number.isInteger(c.number));

  // Get duplicates from manga data
  const duplicateChapters = manga.duplicateChapters || [];

  // Check for chapter count mismatch
  const uniqueCount = manga.uniqueChapters || new Set(chapters.map(c => c.number)).size;
  ...
  const unreadDownloaded = downloadedChapters.filter(n => !readChapters.has(n));

  // Count unread chapters that are NOT downloaded (for "Download Unread" button)
  const unreadNotDownloaded = chapters.filter(c => !readChapters.has(c.number) && !downloadedSet.has(c.number));
  ...
  // Pagination - apply to unique chapter groups
  ...
  const chapterItems = paginatedGroups.map(([num, versions]) => {
  ...
  // Count undownloaded (excluding deleted)
