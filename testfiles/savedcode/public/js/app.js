...
async function downloadUnreadChapters(mangaId) {
  try {
    ...
    // Find chapters that are not read, not downloaded, and not hidden
    const availableChapters = manga.chapters
      .filter(c => !readChapters.has(c.number) && !downloadedChapters.has(c.number) && !deletedUrls.has(c.url));
    ...
    <div class="reader favorites-reader" id="reader">
      <div class="reader-header" id="reader-header">
        <div class="reader-title">
          <span style="color:var(--accent);">⭐ ${listName}</span>
          <span style="margin-left:15px;">${fav.mangaTitle} - Ch.${fav.chapterNum}</span>
        </div>
      ...
      <div class="reader-content manga-mode" id="reader-content" onclick="handleFavoritesClick(event)">
        ${imageHtml}
      </div>
      
      <div class="reader-footer" id="reader-footer">
        <button class="btn btn-secondary btn-small" onclick="prevFavorite()">← Prev</button>
        <span class="page-indicator">${index + 1} / ${favorites.length}</span>
        <button class="btn btn-secondary btn-small" onclick="nextFavorite()">Next →</button>
        <button class="btn btn-danger btn-small" onclick="removeCurrentFavorite()" title="Remove from favorites">🗑️</button>
      </div>
    </div>
  `;
}
...
function prevFavorite() {
  const favorites = state.favorites[state.currentFavoriteList];
...
function nextFavorite() {
  const favorites = state.favorites[state.currentFavoriteList];
  if (state.currentFavoriteIndex < favorites.length - 1) {
    state.currentFavoriteIndex++;
    renderFavoritesReader();
  }
}
...
async function removeCurrentFavorite() {
  ...
  if (confirm('Remove this page from favorites?')) {
    await removeFavorite(listName, index);
    ...
    if (state.currentFavoriteIndex >= favorites.length) {
...
async function showChapterContextMenu(mangaId, chapterNum, event) {
  ...
  popup.innerHTML = `
    <div class="context-menu-items">
      ...
      ` : ''}
      <button class="context-menu-item danger" onclick="removeChapterEntry('${mangaId}', ${chapterNum})">
        🗑️ Remove Chapter Entry
      </button>
    </div>
  `;
...
async function quickCheckUpdates(id) {
  ...
  try {
    ...
    if (result.hasUpdates) {
      showToast(`Found ${result.newChaptersCount} new chapters! Use full check to update.`, 'success');
    } else {
...
async function quickCheckAllUpdates() {
  ...
  try {
    ...
    if (updatesFound > 0) {
      ...
      showToast(`Found updates: ${titles}. Use Full Check to update.`, 'success');
    } else {
...
async function downloadAllChapters(id) {
  try {
    ...
    // Find chapters not downloaded and not deleted
    const availableChapters = manga.chapters
      .filter(c => !downloadedChapters.has(c.number) && !deletedUrls.has(c.url));
