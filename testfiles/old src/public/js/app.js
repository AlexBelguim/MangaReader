...
async function showMangaDetail(params) {
  ...
  try {
    // Load manga and categories in parallel
    ...
    // Reset chapter list pagination when loading a new manga
    state.chapterListPage = 0;
...
async function showReader(params) {
  ...
  try {
    ...
    // Ensure trophy pages are loaded
    if (!state.trophyPages || !state.trophyPages[mangaId]) {
      console.log('Trophy pages missing for manga, reloading...');
      await loadTrophyPages();
    }
...
async function loadFavorites() {
  try {
    const [favData, trophies] = await Promise.all([
      API.getFavorites(),
      API.getTrophiesList()
    ]);

    state.favorites = favData.favorites || {};
    state.favoriteListOrder = favData.listOrder || Object.keys(state.favorites);

    // Add Trophies list
    if (trophies && trophies.length > 0) {
      state.favorites['Trophies'] = trophies;
      if (!state.favoriteListOrder.includes('Trophies')) {
        // Add to the beginning of the list
        state.favoriteListOrder.unshift('Trophies');
      }
    }
  } catch (e) {
...
function renderFavoritesReader() {
  ...
  if (fav.displayMode === 'single' || fav.displaySide === 'both') {
    ...
    if (fav.displayMode === 'double' && images.length === 2) {
      ...
      if (fav.isTrophy) {
        // Wrapper approach for trophies
        imageHtml = `
          <div class="spread favorite-spread trophy-wrapper">
      ...
      const trophyClass = fav.isTrophy ? ' trophy' : '';
      imageHtml = `
        <div class="spread favorite-spread single">
          <img src="${images[0]}" class="spread-page${trophyClass}" alt="Page">
        </div>
      `;
    }
    ...
    imageHtml = `
      <div class="spread favorite-spread single">
        <img src="${imgPath || images[0]}" class="spread-page${trophyClass}" alt="Left">
      </div>
    `;
    ...
    imageHtml = `
      <div class="spread favorite-spread single">
        <img src="${imgPath || images[0]}" class="spread-page${trophyClass}" alt="Right">
      </div>
    `;
  }
      ...
      ? (() => {
        ...
        const blocks = items.map((item, idx) => {
          ...
          if (imagesToShow.length === 2) {
            ...
            const leftClass = item.isTrophy ? 'trophy-image left-side' : '';
            const rightClass = item.isTrophy ? 'trophy-image right-side' : '';
            ...
            return `
               <div id="fav-block-${idx}" style="margin-bottom:10px; display:flex; justify-content:center; max-width:${zoom}%; margin-left:auto; margin-right:auto;">
                 <img src="${leftImg}" class="${leftClass}" style="width:50%; height:auto; display:block;" loading="lazy">
                 <img src="${rightImg}" class="${rightClass}" style="width:50%; height:auto; display:block;" loading="lazy">
               </div>
             `;
          } else {
            // Standard vertical rendering
            const singleClass = item.isTrophy ? 'trophy-image' : '';
            const imagesHtml = imagesToShow.map(imgSrc =>
              `<img src="${imgSrc}" class="${singleClass}" style="max-width:${zoom}%; display:block; margin: 0 auto; margin-bottom:0;" loading="lazy">`
            ).join('');
...
function previewPageSlider(value) {
  ...
  if (state.readerSettings.mode === 'webtoon') {
    indicator.textContent = `Page ${page + 1} / ${state.currentImages.length}`;
  } else {
    const trophyInfo = getCurrentTrophyPages();
    const lastPageSingle = getChapterLastPageSingle();
...
function updatePageSlider() {
  ...
  if (state.readerSettings.mode === 'webtoon') {
    ...
    indicator.textContent = `Page ${currentIndex + 1} / ${images.length}`;
  } else {
    // Manga mode
...
function setupWebtoonScrollListener() {
  ...
  content.addEventListener('scroll', () => {
    ...
    indicator.textContent = `Page ${currentPage} / ${images.length}`;
  });
}
...
function updateMangaPage() {
  ...
  const leftTrophy = leftIdx >= 0 && isTrophyPage(leftIdx);
  const rightTrophy = rightIdx >= 0 && isTrophyPage(rightIdx);
  const isTrophySpread = leftTrophy || rightTrophy;

  let leftHtml = leftImg ? `<img src="${leftImg.url}" alt="Page" class="manga-page left" data-page="${leftIdx}">` : '';
  let rightHtml = rightImg ? `<img src="${rightImg.url}" alt="Page" class="manga-page right" data-page="${rightIdx}">` : '';

  // Handle next chapter image for link mode
  if (nextChapterImg) {
    if (direction === 'rtl') {
      // RTL: next chapter image on left
      ...
      // LTR: next chapter image on right
  ...
  // Use wrapper rendering for trophy spreads
  if (isTrophySpread && leftImg && rightImg) {
    if (direction === 'rtl') {
      content.innerHTML = `<div class="trophy-wrapper">${rightHtml}${leftHtml}</div>`;
    } else {
      content.innerHTML = `<div class="trophy-wrapper">${leftHtml}${rightHtml}</div>`;
    }
  } else {
    // Normal rendering (single images also handled here, trophy class added to img if single)
    if (isTrophySpread && (leftImg || rightImg)) {
      // Single trophy page
      if (leftImg) leftHtml = leftHtml.replace('manga-page', 'manga-page trophy');
      if (rightImg) rightHtml = rightHtml.replace('manga-page', 'manga-page trophy');
    }
...
async function showChapterContextMenu(mangaId, chapterNum, event) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      hiddenCount = result.hiddenVersions?.length || 0;
    }
  } catch (e) { }
  ...
  for (const chapterNum of downloadedChapters) {
    ...
    if (Array.isArray(versions) && versions.length >= 2) {
      try {
        ...
        console.error(`Failed to load version details for chapter ${chapterNum}:`, error);
      }
    }
  }
}
...
function renderAvailableBookmarks(bookmarks, seriesId) {
  ...
  list.innerHTML = bookmarks.map(b => {
    ...
    const coverUrl = b.localCover
      ? `/api/bookmarks/${b.id}/covers/${encodeURIComponent(b.localCover.split(/[/\\]/).pop())}`
      : b.cover;
    return `
