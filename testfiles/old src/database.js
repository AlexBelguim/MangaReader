...
export function initDatabase() {
  ...
  db.exec(`
    ...
    -- Chapter settings (cover pair, linked pages)
...
export const chapterSettingsDb = {
  getAll() {
    const db = getDb();
    const rows = db.prepare('SELECT bookmark_id, chapter_number, first_page_single, last_page_single FROM chapter_settings').all();
  ...
  save(bookmarkId, chapterNumber, settings) {
    ...
    db.prepare(`
      INSERT OR REPLACE INTO chapter_settings (bookmark_id, chapter_number, first_page_single, last_page_single)
      VALUES (?, ?, ?, ?)
    `).run(
      bookmarkId,
      chapterNumber,
  ...
  saveAll(settingsData) {
    ...
    const insert = db.prepare('INSERT INTO chapter_settings (bookmark_id, chapter_number, first_page_single, last_page_single) VALUES (?, ?, ?, ?)');

    db.transaction(() => {
      for (const [mangaId, chapters] of Object.entries(settingsData)) {
        for (const [chNum, settings] of Object.entries(chapters)) {
          insert.run(mangaId, parseFloat(chNum), settings.firstPageSingle ? 1 : 0, settings.lastPageSingle ? 1 : 0);
        }
      }
    })();
