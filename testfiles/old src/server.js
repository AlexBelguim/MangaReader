...
// Global task queue to prevent parallel scraping
const taskQueue = {
  queue: [],
  tasks: new Map(), // Track all tasks by ID
  isProcessing: false,
  currentTask: null,
  nextTaskId: 1,
  ...
  async addAndWait(item) {
    ...
    const taskInfo = {
      ...
      mangaTitle: item.mangaTitle || null,
      status: 'queued',
      createdAt: Date.now(),
      startedAt: null,
      completedAt: null,
      ...
      this.add({
        ...
        execute: async () => {
          try {
            ...
            setTimeout(() => this.tasks.delete(taskId), 30000);
            resolve(result);
          } catch (error) {
            taskInfo.status = 'error';
            taskInfo.error = error.message;
            taskInfo.completedAt = Date.now();
            // Remove failed tasks after 30 seconds
            setTimeout(() => this.tasks.delete(taskId), 30000);
            reject(error);
          }
        }
  ...
  async process() {
    ...
    while (this.queue.length > 0) {
      ...
      this.currentTask = item.type || 'task';
      try {
        await item.execute();
      } catch (error) {
...
// Get all queue tasks (scrape tasks)
...
app.post('/api/bookmarks', async (req, res) => {
  try {
    ...
    const result = await taskQueue.addAndWait({
      type: 'scrape',
      description: `Adding manga from ${scraper.websiteName}`,
      execute: async () => {
        console.log(`[API] Using scraper: ${scraper.websiteName}`);
        const mangaInfo = await scraper.getMangaInfo(url);
        console.log(`[API] Scraped: ${mangaInfo.title}`);

        const result = await bookmarkDb.add(mangaInfo);
        console.log(`[API] Bookmark saved: ${result.success}`);

        // Download cover locally if available
        if (result.success && mangaInfo.cover) {
          console.log(`[API] Downloading cover...`);
          try {
            ...
            if (coverResult) {
              // Update bookmark with local cover path
              await bookmarkDb.update(result.bookmark.id, {
                localCover: coverResult.path,
                remoteCover: coverUrl
              });
              result.bookmark.localCover = coverResult.path;
              console.log(`[API] Cover saved`);
            }
          } catch (coverError) {
            console.error(`[API] Cover download failed:`, coverError.message);
            // Continue anyway - cover is not critical
          }
        }

        // Save artists if available (from nhentai or other scrapers)
        if (result.success && mangaInfo.artists && mangaInfo.artists.length > 0) {
          console.log(`[API] Saving artists: ${mangaInfo.artists.join(', ')}`);
          artistDb.setForBookmark(result.bookmark.id, mangaInfo.artists);
          result.bookmark.artists = mangaInfo.artists;
        }
...
app.post('/api/bookmarks/:id/download', async (req, res) => {
  try {
    ...
    if (all) {
      // Download all chapters NOT already downloaded, NOT deleted, and NOT excluded
      ...
      // Download specific chapters (user explicitly chose, but still respect excluded)
...
app.post('/api/bookmarks/:id/scan-local', async (req, res) => {
  try {
    ...
    if (chaptersToMerge) {
      ...
      updateData.uniqueChapters = new Set(chaptersToMerge.map(c => c.number)).size;
    } else {
      // Always recalculate uniqueChapters to fix any desync
      const currentChapters = bookmark.chapters || [];
      updateData.uniqueChapters = new Set(currentChapters.map(c => c.number)).size;
    }
...
// Get aggregated trophies with image paths for favorites view
app.get('/api/trophies-list', async (req, res) => {
  try {
    const trophies = trophyDb.getAll();
    const bookmarks = await bookmarkDb.getAll();
    const bookmarksMap = bookmarks.reduce((acc, b) => { acc[b.id] = b; return acc; }, {});
    const result = [];

    for (const [mangaId, chapters] of Object.entries(trophies)) {
      const manga = bookmarksMap[mangaId];
      if (!manga) continue;

      for (const [chapterNumStr, pagesMap] of Object.entries(chapters)) {
        const chapterNum = parseFloat(chapterNumStr);
        if (!manga.downloadedChapters.includes(chapterNum)) continue;

        // Get local images for this chapter
        const images = await downloader.getLocalChapterImages(
          manga.title,
          chapterNum,
          manga.alias
        );
        ...
        // Track processed pages to avoid duplicates for spreads
        const processedPages = new Set();
        const pageIndices = Object.keys(pagesMap).map(k => parseInt(k)).sort((a, b) => a - b);

        for (const pageIndex of pageIndices) {
          if (processedPages.has(pageIndex)) continue;

          const trophyData = pagesMap[pageIndex];
          if (images[pageIndex]) {
            // Determine images for this trophy (handle spreads)
            let imagePaths = [];
            let pagesInTrophy = [];

            if (trophyData.pages && trophyData.pages.length > 0) {
              imagePaths = trophyData.pages.map(i => images[i]).filter(Boolean);
              pagesInTrophy = trophyData.pages;
            } else {
              imagePaths = [images[pageIndex]];
              pagesInTrophy = [pageIndex];
            }

            // Mark these pages as processed
            pagesInTrophy.forEach(p => processedPages.add(p));
            ...
            result.push({
              ...
              chapterUrl: null,
              pageIndices: pagesInTrophy,
              displayMode: pagesInTrophy.length > 1 ? 'double' : 'single',
              displaySide: 'both',
              imagePaths: imagePaths,
              createdAt: Date.now(),
              tags: ['Trophy'], // Add the Trophy tag
              isTrophy: true // Marker for UI styling
            });
          }
        }
      }
    }
    // Sort by most recent (approximated by internal order or just reverse)
    res.json(result.reverse());
  } catch (error) {
    console.error('Error fetching trophies list:', error);
    res.status(500).json({ error: error.message });
  }
});
