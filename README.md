...
- **Magnet links**: the Find volumes dialog takes a pasted magnet link or `.torrent` URL from anywhere; it goes to qBittorrent with the manga category and save path, is tracked on the queue page linked to the series, and imports like a searched release. The dialog also has a per-download switch for importing automatically versus reviewing first; an explicit review import replaces an automatic one that is still waiting in the queue.
- **AniList by hand**: the Track dialog shows AniList's chapter, the highest chapter read here, and a field to set the chapter directly - for progress made through volumes or elsewhere. Chapters up to it are marked read here, and reading on syncs from there. Finishing a release volume that has chapters assigned syncs those too.
- **Find more like this** (series page): search the site(s) the series' manga come from - or every site - with an editable title, and add a result straight into the series. A new title is scraped and filed there with the categories and check schedule of the series' first entry; a title already in the library is just linked.
